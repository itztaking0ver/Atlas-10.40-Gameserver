...
namespace Bots {
    void SpawnPlayerBot(AActor* OverrideSpawnLoc = nullptr, PlayerBots::EBotState StartingState = PlayerBots::EBotState::Warmup) {
        ...
        if (true)
        {
            int Ret = UAtlas->NextBotTeamIndex;

            ++UAtlas->CurrentBotsOnTeam;

            if (UAtlas->CurrentBotsOnTeam == UAtlas->MaxPlayersPerTeam)
            {
                UAtlas->NextBotTeamIndex++;
                UAtlas->CurrentBotsOnTeam = 0;
            }
